...
public class DangerSourceService  extends CommonMethod{
    ...
    public static void addDangerSource(String path, By mc){
        ...
        Boolean total=driver.findElement(DangerSourcePage.COUNT).isDisplayed();
        int t;
        if(total.equals(true)){
             t= count(DangerSourcePage.COUNT);
        }else {
            t=0;
        }


        driver.findElement(DangerSourcePage.PROJECTLIST).click();//点击项目管理
        new WebDriverWait(driver,30).until(ExpectedConditions.presenceOfElementLocated(DangerSourcePage.FIRSTPROJECT));
        ...
        driver.findElement(DangerSourcePage.INITEM).click();
        try {
            Thread.sleep(3000);
        } catch (InterruptedException e) {
            e.printStackTrace();
...
//        total1=total1.replaceAll("共","").replaceAll("条","").trim();//获取中间条数， 文字替换成空格并去除空格+
    ...
    public static void dangerDetail(String pathfile,By close){
        try {
            Thread.sleep(2000);
        } catch (InterruptedException e) {
            e.printStackTrace();
